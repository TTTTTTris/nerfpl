...
class NeRFSystem(LightningModule):
    ...
    def prepare_data(self):     # OK
        ...
        self.train_dataset = dataset(self.hparams.root_dir, Ex, self.hparams.operator)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.batch_cnt == 0 or self.batch_cnt % 1000 == 999:
            ...
            axes[0].imshow(gt1.cpu().view(w1,h1).detach().numpy())
            axes[0].set_title('gt1')
            axes[1].imshow(image_pred1.cpu().view(w1,h1).detach().numpy())
            axes[1].set_title('psnr: %0.2f'%(psnr1))

            axes[2].imshow(gt2.cpu().view(w1,h1).detach().numpy())
            axes[2].set_title('gt2')
            axes[3].imshow(image_pred2.cpu().view(w1,h1).detach().numpy())
...
if __name__ == '__main__':
    ...
    E = torch.rand(1,1,kernel,kernel).cuda()
    Ex = E.cpu()
    # print(E)
    # print(Ex)
    # 
    ...
    #
    hparams.operator    = 'conv' # fit, add, sub, mul, div, ln, etc 
    ...
    hparams.exp_name    = hparams.exp_name + '_' + hparams.modelType + '_' + hparams.operator + str(kernel) + 'x'
